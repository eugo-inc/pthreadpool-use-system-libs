...
2. Fallback (eugo-kb unavailable): read
   ...
   `eugo-skills pull --from /opt/eugo/athena --cache --name eugo-upstream-merge-base`.
   `--from` is athena's checkout AT ITS HEAD: `/opt/eugo/athena` on the
   eugo box; elsewhere, the clone whose `git remote get-url origin`
   names `eugo-inc/athena` (with or without `.git`). The pull copies
   that checkout's working tree as it stands, uncommitted edits
   included, so a stale or dirty checkout serves stale text.
...
# DURABLE HOME of every value below: .eugo.toml [skills.adapters."eugo-upstream-merge"] — this block is REGENERATED by `eugo-skills pull --as-adapter`; an edit made here is overwritten.
params:
