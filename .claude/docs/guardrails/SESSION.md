<!-- guardrails-kit: v1.1.2 (§1996 iron-rule back-pointers say athena's) | Editing this file? Read .claude/docs/guardrails/_FORMAT.md first. Never paraphrase kit text. -->
...
> eugo reconciliation: this repo tracks the LIVE running/done/next list in the **TodoWrite** tool (per athena's CLAUDE.md ## Project), not a file. `.claude/docs/guardrails/STATE.md` is the OPTIONAL persistent store for cross-compaction Constraints / Facts / Decisions only — create it via S2 when a task spans a compaction. Post-compaction recovery also reads the approved plan file under `~/.claude/plans/`, `git log --oneline -10` + `git status`, and (overnight runs) the `.claude/data/overnight/` breadcrumbs. A `## Done` RESULT that is durable TECHNICAL knowledge beyond this repo also gets ONE `donate_fact` over eugo-kb (§facts-donate; run-metadata never qualifies).
