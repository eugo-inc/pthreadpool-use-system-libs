<!-- guardrails-kit: v1.2.3-eugo stub (§2001 harness-agnostic task widget; §1996 back-pointers say athena's) | full playbook: the superpowers skills. Editing? Read .claude/docs/guardrails/_FORMAT.md first. -->
...
- P9. Task spans a compaction and no .claude/docs/guardrails/STATE.md exists? Create it per .claude/docs/guardrails/SESSION.md S2. The LIVE running/done/next list belongs in the native task widget (Claude Code: `TodoWrite`), not a file — and THIS repo's `CLAUDE.md` names any further recovery sources.
- P10. Every TODO list goes to the user BEFORE the first Edit: one task per item in the native task widget, flipped to completed as each lands. Anything needing the user's ACTION, INPUT or ATTENTION goes through AskUserQuestion the moment it arises — before the work, during it, or in the final answer — never prose, never only a row in a doc. Sole exception: an `INVOLVEMENT: unattended` run FILEs instead of blocking and asks at its close. (Compressed as athena's CLAUDE.md iron rule 13.)
