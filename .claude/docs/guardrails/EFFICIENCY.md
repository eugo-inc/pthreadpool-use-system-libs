...
- E14. Your next sentence contains "probably / presumably / likely / I assume / should be" about this repo's code? Delete the sentence; run the Grep or Read that answers it. Guessing about verifiable repo facts is never the efficient move. (Compressed as athena's CLAUDE.md iron rule 10 — keep the two phrase lists byte-identical.)
...
- E6. Between tool calls: at most ONE short line, and only to record a finding or decision. Nothing before a call whose purpose is obvious from its parameters. (E5+E6 compressed as athena's CLAUDE.md iron rule 12.)
...
- E15. Final answer for a task where the user asked for changes (not an explanation/report): at most 10 lines — changed files as path:line + what changed, the verification command and observed result, open risks. VERIFY.md's `Verified:` evidence lines and pasted gate outputs are EXEMPT from the cap. No code blocks unless the user must copy them; no restating the request; no narrating the journey.
...
- E18. Never Read a subagent's raw `.output`/transcript file — one read can overflow the context and end the session; consume its structured return or a jq/grep digest instead.

--- reference ---

## Context is heavy and a long stretch of work remains
Checkpoint before it is too late: Read .claude/docs/guardrails/SESSION.md and bring .claude/docs/guardrails/STATE.md up to the present; route any command expected to emit >200 lines to a log file (E10); prefer delegation (E8) for anything wide. Compaction after a checkpoint costs minutes; compaction before one costs the session.
